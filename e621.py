...
class e621(APIinstance):
    ...
    def headers(self):
        return {
            'User-Agent': 'Pekka4597/1.0 (by Pekka4597 on e621)'
        }
...
class Booru:
    ...
    def __init__(self, bot):
        ...
        self.e621_ = e621(bot.session)
    @commands.command()
    async def e621(self, *tags: str):
        """Search e621 for images [nsfw only].
        Enter multiple tags by seperating them with spaces
        """
        await bot.say(embed=await self.e621_.parse_req_tags(*tags))
"""        except:
            logging.error(type(bot))
            logging.error(bot)"""
